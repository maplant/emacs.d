...
(defgroup custom-solarized nil
  "The sanityinc solarized theme pair."
  :group 'appearance
  :prefix "custom-solarized-")

(defcustom custom-solarized-rgb-is-srgb
...
(defmacro custom-solarized--with-colors (mode &rest body)
...
`MODE' should be set to either 'light or 'dark."
  ...
  `(let* ((srgb custom-solarized-rgb-is-srgb)
...
(defmacro custom-solarized--face-specs ()
  ...
  (quote
   (mapcar
    ...
    `(;; Standard font lock faces
      ...
      ;; company
      (company-preview (:foreground ,faint :background ,alt-background))
      ...
      (company-tooltip-selection (:background ,alt-background :bold t))
      ...
      (company-tooltip-annotation-selection (:inherit company-tooltip :foreground ,green :background ,alt-background :bold t))
      (company-scrollbar-bg (:inherit company-tooltip :background ,contrast-background))
      (company-scrollbar-fg (:background ,alt-background))
...
(defmacro custom-solarized--frame-parameter-specs ()
...
(defun custom-solarized (mode)
  "Apply either the dark or the light theme."
  (if (fboundp 'load-theme)
      (let ((name (cond
                    ((eq 'light mode) 'custom-solarized-light)
                    ((eq 'dark mode) 'custom-solarized-dark)
    ...
    (progn
      ...
      (custom-solarized--with-colors
       mode
       (color-theme-install
        `(,(intern (concat "custom-solarized-" (symbol-name mode)))
          ,@(custom-solarized--frame-parameter-specs)
          ,@(custom-solarized--face-specs)))
...
;; ;;;###autoload
;; (defun custom-solarized-dark ()
;;   "Apply the dark solarized theme."
;;   (interactive)
;;   (custom-solarized 'dark))
;; 
;; ;;;###autoload
;; (defun custom-solarized-light ()
;;   "Apply the light solarized theme."
;;   (interactive)
;;   (custom-solarized 'light))


(provide 'custom-solarized)

(custom-solarized--define-theme light)
(custom-solarized--define-theme dark)

;; Local Variables:
;; byte-compile-warnings: (not cl-functions)
;; End:

;;; custom-solarized.el ends here
